...
This began as a Phase 0 document, written to constrain a design rather than to audit an
implementation. Through Phase 4 it has become both: rows carry a **Built:** clause where
the control exists in code and is tested, and say nothing where it does not, so what is
missing is as readable as what is there. It is revisited at the end of each phase, and it
is **not** a substitute for the independent security review, custody architecture review
and penetration test the brief requires before real funds.
...
| ---- | --------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B5.1 | Malware or a polyglot file served to staff    | T, E   | Size and MIME/magic-byte validation, malware scan **before** staff access, private objects, download via time-limited signed URL, served with `Content-Disposition: attachment` and a restrictive CSP from a separate origin. **Not built: there is no scanner.** What exists instead is narrow: three image types by magic bytes, 5 MB, bytes proxied through the API under the resolver's session, and a resolver screen that renders a file that will not decode as a sentence rather than an element. A polyglot that is a valid image is still served |
| B5.2 | Evidence from one dispute readable in another | I      | Object keys are unguessable; every access is authorized against the dispute, never by URL possession alone. Built: keys are UUIDv7 under the trade, and both the party route and the resolver route look the file up through its dispute before answering (`disputes.spec.ts`, AT-6)                                                                                                                                                                                                                                                                       |
...
| B5.4 | Uploads used as free storage / DoS            | D      | Per-dispute count and size caps, per-user rate limit. Built: five files per party per dispute, 5 MB each, JPEG/PNG/WebP by magic bytes, thirty uploads an hour per session                                                                                                                                                                                                                                                                                                                                                                                 |
...
| #    | Threat                                                                | STRIDE | Control                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ---- | --------------------------------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B7.1 | **Malicious or compromised admin resolves disputes to an accomplice** | E, R   | Least-privilege roles (`dispute_resolver` ≠ `withdrawal_approver` ≠ `financial_adjuster`); mandatory reason codes and evidence; append-only audit; resolver cannot be a party to the trade (administrators are a separate realm with no customer account). Built: the role gates every dispute route, the note is mandatory and reaches both parties, and the decision writes an audit event with actor, correlation id, before, after and the evidence ids (`disputes.spec.ts`, AT-8). **Still outstanding: anomaly reporting on resolution patterns per admin** — nothing counts how one resolver decides, and that detection is the half of this control that catches an insider, because prevention is impossible |
...
| B7.5 | Admin reads customer payment details at will                          | I      | Access is logged and attributable; sensitive fields decrypted only on justified access with a reason code. Built: opening a dispute writes `dispute.viewed` with the resolver, the dispute and the request id before the payment instructions are decrypted, and the screen tells the resolver that the look is recorded. The instructions are reachable only through a dispute, never by customer or by search                                                                                                                                                                                                                                                                                                       |
...
| **R-08** | Insider abuses dispute resolution or approval                                                 | Low         | High                   | **Medium**                | Role separation, dual approval, append-only audit, pattern monitoring                | AT-8 (green; pattern monitoring unbuilt)  |
