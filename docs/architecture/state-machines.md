...
| `DETECTED` | Destination is not a known active address, or token is not the configured one, or amount below dust | `UNATTRIBUTED` | System | — | JE-10a *(only if the funds are genuinely under platform control)* | `deposit:{id}:unattributed` | `deposit.unattributed` |
...
- Escrow is funded by whoever gives up USDT (ADR-0004), which on a *buy* offer is the
