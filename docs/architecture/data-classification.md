...
| Presence: whether someone is online, and when they were last seen                             | PUBLIC         | Redis + PostgreSQL          | Shown to signed-in customers beside an advertiser or a counterparty, with no opt-out - the owner's decision, as on Binance. To the minute, because strangers read it                                                                        |
...
- `DISPUTE_RESOLVER` is deliberately **not** on that list, though a resolver does see two
  named customers: the buyer and the seller of the dispute in front of them, embedded in
  that dispute's own response. Deciding between two people requires knowing who they are;
  it does not require being able to walk the directory, and the narrower power is the one
  worth granting (Phase 4, stage 6).
...
| `STORAGE_ACCESS_KEY_ID` / `_SECRET_ACCESS_KEY` | Identity documents and dispute evidence      | Secrets service, scoped IAM role                 | 90d                      | Read every identity document and every piece of dispute evidence                                                                                 |
