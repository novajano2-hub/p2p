...
| **Identity-verification documents** (ID front and back, selfie)                               | **RESTRICTED** | Object storage, private     | Private bucket the API alone writes; the database holds keys, never URLs; an administrator reads via a signed, short-lived address     |
...
| `STORAGE_ACCESS_KEY_ID` / `_SECRET_ACCESS_KEY` | Identity documents now; dispute evidence later | Secrets service, scoped IAM role                 | 90d                      | Read every identity document and, later, all dispute evidence                                                                                    |
