...
The fee is zero at launch, but its *shape* changes the escrow amount and therefore the
...
1. *Interface shape (soon):* does the provider issue per-user deposit addresses natively,
...
2. *Feasibility (before committing to the network):* **does any provider you would
...
you asked for next needs *something*. If you have a name, colors, or a tone in mind, tell
...
| C-6 | "Never promise a universally free withdrawal" vs. the appeal of sponsored gas | User-facing copy is fixed as *"No platform fee. The network or a third party may charge a fee."* until U-2/U-3/U-4 are validated |
