...
| Admin session length: 24 h absolute, 8 h idle      | Before production | Raised from 8 h / 30 min on 2026-09-17 at the owner's request, so hand-testing across both realms is not interrupted. Threat model B7.3 asks for short admin sessions; the production numbers are a deployment decision, not this one. The env schema caps at these values (`ADMIN_SESSION_TTL_HOURS`, `ADMIN_SESSION_IDLE_MINUTES`) |
