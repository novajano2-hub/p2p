...
- **Dark mode is required on every surface, from the first line of markup.** The landing page and any signed-out page follow the operating system (`prefers-color-scheme`). Once a customer has an account they choose system / light / dark, which sets `data-theme="light"` or `data-theme="dark"` on `<html>`; the token layer is already wired for both, so that feature is a preference control and a persisted column, never a restyle. Dark values for the kit: Canvas `#121614`, Surface `#1A201D`, foreground `#E9EDE8`, muted `#A3ADA6`, Border `#2B332F`, accent `#6CC39C` on `#0D1411`.
